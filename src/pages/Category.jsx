...
export const Category = () => {
    ...
    const getCategory = async () => {
        try {
            setLoading(true);
            console.log(await axios.get(`${BASE_URL}${APP_API.category}`))
    ...
    const saveCategory = async () => {
        ...
        try {
            if (editMode && editId) {
                ...
                const res = await axios.post(`${BASE_URL}${APP_API.category}`, formData, {
                    headers: {
                        'Content-Type': undefined, // 👉 majburan o‘chirish
                    },
                });
                alert(res.message)
            }
            ...
            axios.isAxiosError(err) ? err.response?.data : 'Nomaʼlum xatolik yuz berdi';
                            ...
                            <tr key={cat._id} className="border-t hover:bg-yellow-50">
                                ...
                                <td className="p-3">
                                    <div className="w-16 h-12 overflow-hidden border rounded">
                                        <img src={`${cat.imageUrl}`} alt="img" className="w-full h-full object-cover" />
                                    </div>
                                </td>
